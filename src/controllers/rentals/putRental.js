...
export async function putRental(req,res) {
    try{
        ...
        let daysLate = Math.floor((date1.getTime()+3*3600*1000-date2.getTime())/(24*3600*1000));
        if(daysLate<0) {
            daysLate = 0;
        }
        const gamePrice = await connection.query(`SELECT "pricePerDay" FROM games WHERE id = $1`, [rentalInfos.gameId]);
